...
# --- 2. LA NUEVA LAMBDA (GENERADOR DE SEÑALES) ---
data "archive_file" "signals_zip" {
  type        = "zip"
  source_file = "src/signal_generator.py" # Asegúrate de crear este archivo
  output_path = "signals_lambda.zip"
}

resource "aws_lambda_function" "signal_generator" {
  filename         = data.archive_file.signals_zip.output_path
  function_name    = "crypto-signal-generator"
  role             = aws_iam_role.lambda_exec.arn # Reusamos el rol existente
  handler          = "signal_generator.handler"
  runtime          = "python3.9"
  source_code_hash = data.archive_file.signals_zip.output_base64sha256
  timeout          = 30

  environment {
    variables = {
      SIGNALS_BUCKET_NAME = aws_s3_bucket.signals_bucket.bucket
    }
  }
}

# --- 3. PERMISOS: S3 Trigger -> Lambda ---
resource "aws_lambda_permission" "allow_bucket" {
  statement_id  = "AllowExecutionFromS3"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.signal_generator.arn
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.data_bucket.arn # El bucket donde subes el analysis
}

# --- 4. EL DISPARADOR (EVENT NOTIFICATION) ---
resource "aws_s3_bucket_notification" "bucket_notification" {
  bucket = aws_s3_bucket.data_bucket.id

  lambda_function {
    lambda_function_arn = aws_lambda_function.signal_generator.arn
    events              = ["s3:ObjectCreated:*"]
    # Solo dispara si se actualiza este archivo específico
...
# --- 5. PERMISOS IAM ADICIONALES ---
# Necesitamos que la Lambda pueda escribir en el NUEVO bucket de señales
resource "aws_iam_role_policy" "lambda_signals_policy" {
  name = "lambda_signals_policy"
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.signals_bucket.arn}/*", # Escribir en señales
          "${aws_s3_bucket.data_bucket.arn}/*"     # Leer del analysis
        ]
      }
    ]
  })
